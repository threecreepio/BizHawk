...
namespace BizHawk.Client.EmuHawk
{
	...
	public class DisplayManager : IDisposable
	{
		...
		public DisplayManager(PresentationPanel presentationPanel)
		{
			...
			using (var xml = typeof(Program).Assembly.GetManifestResourceStream("BizHawk.Client.EmuHawk.Mono.courier16px.fnt"))
			{
				using var tex = typeof(Program).Assembly.GetManifestResourceStream("BizHawk.Client.EmuHawk.Mono.courier16px_0.png");
				TheOneFont = new StringRenderer(GL, xml, tex);
			}

			using (var gens =
				typeof(Program).Assembly.GetManifestResourceStream("BizHawk.Client.EmuHawk.Mono.gens.ttf"))
			{
				LoadCustomFont(gens);
			}

			using (var fceux =
				typeof(Program).Assembly.GetManifestResourceStream("BizHawk.Client.EmuHawk.Mono.fceux.ttf"))
			{
